...
| Скрипт | Действие |
|--------|----------|
| `npm run dev` | Vite в режиме разработки. Запросы проксируются на **http://localhost:4219** — сервер должен быть запущен на этом порту (по умолчанию так и есть). |
...
| `npm run preview` | Раздача собранного **../web/** через Vite (проверка продакшен-сборки). |
| `npm run lint` | ESLint по `.ts`/`.tsx`. |
| `npm run type-check` | Проверка типов без сборки (`tsc --noEmit`). |
