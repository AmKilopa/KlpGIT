...
**Возможности:** история коммитов, переключение веток, stash (спрятать/вернуть изменения), многострочные сообщения коммита (Ctrl+Enter), тёмная и светлая тема, горячие клавиши (? — подсказка, Escape — закрыть, Shift+S — коммит, Shift+R — обновить).
...
1. Установка зависимостей и сборка (один раз):
   ```bash
   npm install
   npm run build
   ```
